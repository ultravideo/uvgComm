...
bool OpusEncoderFilter::init()
{
  ...
  if(error)
  {
    printDebugObject(DEBUG_WARNING, this, "Iniating", "Failed to initialize opus encoder.",
...
void OpusEncoderFilter::process()
{
  ...
  while(input)
  {
    ...
    for(uint32_t i = 0; i < input->data_size; i += format_.bytesPerFrame()*numberOfSamples_)
    {
      ...
      if(len <= 0)
      {
        printDebugObject(DEBUG_WARNING, this, "Process", "Failed to encode audio",
    ...
    {
      printDebugObject(DEBUG_WARNING, this, "Process", "Failed to encode audio frame.",
