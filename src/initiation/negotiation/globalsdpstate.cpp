...
std::shared_ptr<SDPMessageInfo> GlobalSDPState::generateInitialSDP(QHostAddress localAddress)
{
...
std::shared_ptr<SDPMessageInfo>
    GlobalSDPState::generateResponseSDP(SDPMessageInfo &sdpOffer,
                                        ...
                                        uint32_t sessionID)
{
  ...
  if(localSuggestion == nullptr)
  {
    ...
    sdp->sessionName = sdpOffer.sessionName;
  }
  else // TODO: This crashes sometimes
  {
    ...
    ice_->respondToNominations(sdp->candidates, sdpOffer.candidates, sessionID);
    ...
    if (nominated.audio.first != nullptr && nominated.audio.second != nullptr)
    {
      ...
      setMediaPair(sdpOffer.media[0], nominated.audio.first->remote);
    }

    if (nominated.video.first != nullptr && nominated.video.second != nullptr)
    {
      setMediaPair(sdp->media[1],      nominated.video.first->local);
      setMediaPair(sdpOffer.media[1], nominated.video.first->remote);
    }
  }

  sdp->sess_v = sdpOffer.sess_v + 1;
